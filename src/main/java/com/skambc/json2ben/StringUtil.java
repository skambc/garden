...
public final class StringUtil {
	...
	public static String formatTimestamp(String strdate) {
		if (strdate != null
				&& (strdate.length() < 25 || strdate.split("\\ ").length < 3)) {
			strdate = "";
		}
		return strdate;
	}
	...
	public static String replaceStr(String inputStr, String oldStr,
			String newStr) {
		String retutnstr = "";
		int first = inputStr.indexOf(oldStr);
		int last = inputStr.lastIndexOf(oldStr);
		if (first != last) {
			retutnstr = inputStr.substring(0, first) + newStr
					+ inputStr.substring(first + 1, last) + newStr;
